...
export default {
  async fetch(request, env, ctx) {
    ...
    if (isAIBot(ua)) {
      try {
        const serveUrl = 'https://app.meteoria.ai/api/crawler-analytics/bot-content/serve?token=mt_73203b4fe7ec4ab6bdb54a7361d2822f&path=' + encodeURIComponent(requestUrl.pathname);
...
async function forwardToMeteoria(request, requestUrl, ua, opts) {
  ...
  await fetch('https://app.meteoria.ai/api/crawler-analytics/server-track', {
    ...
    body: JSON.stringify({
      token: 'mt_73203b4fe7ec4ab6bdb54a7361d2822f',
